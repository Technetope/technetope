...
UnifiedConfig loadConfig(const std::filesystem::path& configPath) {
    UnifiedConfig config = getDefaultConfig();
    ...
    try {
        ...
        if (root.contains("osc")) {
            ...
            if (osc.contains("broadcast")) {
                config.osc.broadcast = osc["broadcast"].get<bool>();
            }
        ...
        if (root.contains("swarm")) {
            ...
            if (swarm.contains("enabled")) {
                config.swarm.enabled = swarm["enabled"].get<bool>();
            }
        ...
        if (root.contains("scheduler")) {
            ...
            if (scheduler.contains("enabled")) {
                config.scheduler.enabled = scheduler["enabled"].get<bool>();
            }
            if (scheduler.contains("timeline_path")) {
                config.scheduler.timelinePath = scheduler["timeline_path"].get<std::string>();
            }
            if (scheduler.contains("lead_time")) {
                config.scheduler.leadTime = scheduler["lead_time"].get<double>();
            }
            if (scheduler.contains("bundle_spacing")) {
                config.scheduler.bundleSpacing = scheduler["bundle_spacing"].get<double>();
            }
            if (scheduler.contains("target_map")) {
                config.scheduler.targetMapPath = scheduler["target_map"].get<std::string>();
            }
            if (scheduler.contains("default_targets")) {
                if (scheduler["default_targets"].is_array()) {
                    config.scheduler.defaultTargets = scheduler["default_targets"].get<std::vector<std::string>>();
                } else if (scheduler["default_targets"].is_string()) {
                    // カンマ区切りの文字列をパース
                    std::string targetsStr = scheduler["default_targets"].get<std::string>();
                    ...
                    while (std::getline(ss, item, ',')) {
                        ...
                        if (!item.empty()) {
                            config.scheduler.defaultTargets.push_back(item);
                        }
                    }
                }
            }
