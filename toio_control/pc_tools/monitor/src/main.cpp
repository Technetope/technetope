...
double argumentToSeconds(const toio_control::osc::Argument& arg) {
...
std::string describeArgument(const toio_control::osc::Argument& arg) {
    return std::visit(
        [](const auto& value) -> std::string {
            ...
            } else if constexpr (std::is_same_v<T, toio_control::osc::Blob>) {
...
HeartbeatData parseHeartbeat(const toio_control::osc::Message& message) {
...
std::optional<json> processMessage(const toio_control::osc::Message& message,
                    const MonitorOptions& options,
                    std::unordered_map<std::string, DeviceStats>& stats,
                    std::ofstream* csvStream,
                    toio_control::common::DeviceRegistry* registry) {
...
std::vector<json> processPacket(const toio_control::osc::Packet& packet,
                   const MonitorOptions& options,
                   std::unordered_map<std::string, DeviceStats>& stats,
                   std::ofstream* csvStream,
                   toio_control::common::DeviceRegistry* registry) {
    std::vector<json> events;
    auto handle = [&](const toio_control::osc::Message& msg) {
    ...
    if (const auto* message = std::get_if<toio_control::osc::Message>(&packet)) {
        handle(*message);
    } else if (const auto* bundle = std::get_if<toio_control::osc::Bundle>(&packet)) {
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        toio_control::common::DeviceRegistry registry(options.registryPath);
        registry.load();
        ...
        toio_control::osc::IoContextRunner runner;
        WebSocketBroadcaster* wsRaw = wsBroadcaster.get();

        toio_control::osc::OscListener listener(
            runner.context(),
            toio_control::osc::OscListener::Endpoint(listenAddress, options.port),
            [&](const toio_control::osc::Packet& packet, const toio_control::osc::OscListener::Endpoint&) {
                std::vector<json> events;
                {
