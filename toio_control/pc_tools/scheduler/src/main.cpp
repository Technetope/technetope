...
struct OscMaterial {
    toio_control::osc::OscEncryptor::Key256 key;
    toio_control::osc::OscEncryptor::Iv128 iv;
};
...
struct SchedulerOptions {
    ...
    std::filesystem::path oscConfigPath{"toio_control/secrets/osc_config.json"};
    toio_control::osc::OscEncryptor::Key256 oscKey{};
    toio_control::osc::OscEncryptor::Iv128 oscIv{};
};
...
void printArgument(const toio_control::osc::Argument& arg) {
    std::visit([](const auto& value) {
        using T = std::decay_t<decltype(value)>;
        if constexpr (std::is_same_v<T, toio_control::osc::Blob>) {
...
int main(int argc, char** argv) {
    ...
    try {
        toio_control::scheduler::SchedulerConfig config;
        ...
        if (!opts.baseTimeIso.empty()) {
            config.baseTime = toio_control::scheduler::SchedulerController::parseBaseTime(opts.baseTimeIso);
        }
        ...
        toio_control::scheduler::SchedulerController controller;
