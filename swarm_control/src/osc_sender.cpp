...
OscSender::OscSender(const std::string& address, int port)
    ...
    try {
        ...
        acoustics::osc::OscSender::Endpoint endpoint(addr, static_cast<unsigned short>(port));
        sender_ = std::make_unique<acoustics::osc::OscSender>(
...
void OscSender::sendMessage(const acoustics::osc::Message& message) {
...
void OscSender::sendBundle(const acoustics::osc::Bundle& bundle) {
    if (!connected_ || !sender_) {
        return;
    }
...
void OscSender::sendTargets(const std::vector<TargetPosition>& targets) {
    acoustics::osc::Bundle bundle;
    bundle.timetag = acoustics::osc::toTimetag(std::chrono::system_clock::now());
