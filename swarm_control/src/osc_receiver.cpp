...
OscReceiver::OscReceiver(int listenPort)
    ...
    try {
        ioContextRunner_ = std::make_unique<acoustics::osc::IoContextRunner>();
        ioContextRunner_->start();
        
        asio::ip::address listenAddress = asio::ip::address_v4::any();
        acoustics::osc::OscListener::Endpoint endpoint(listenAddress, static_cast<unsigned short>(listenPort_));
        
        listener_ = std::make_unique<acoustics::osc::OscListener>(
            ioContextRunner_->context(),
            endpoint,
            [this](const acoustics::osc::Packet& packet, 
...
void OscReceiver::enableEncryption(const acoustics::osc::OscEncryptor::Key256& key,
...
void OscReceiver::handlePacket(const acoustics::osc::Packet& packet, 
                               const acoustics::osc::OscListener::Endpoint& endpoint) {
    try {
        if (packet.isBundle()) {
...
void OscReceiver::processMessage(const acoustics::osc::Message& message) {
...
void OscReceiver::processAnnounce(const acoustics::osc::Message& message) {
...
void OscReceiver::processHeartbeat(const acoustics::osc::Message& message) {
...
void OscReceiver::processPosition(const acoustics::osc::Message& message) {
